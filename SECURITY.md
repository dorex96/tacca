...
API keys are written only to `flutter_secure_storage` (iOS Keychain / Android Keystore) by `SecureSettingsRepository`, one entry per provider (`ai.openrouter.apiKey`, `ai.anthropic.apiKey`, `ai.google.apiKey`). A key is never written to ObjectBox, never included in an export, never logged, and never leaves the device except as the authentication header (`Authorization` for OpenRouter, `x-api-key` for Anthropic, `x-goog-api-key` for Google) of a request to *that same provider*, triggered by the user. The repository ships no key of its own, and `android/key.properties`, which holds the Android signing credentials on the maintainer's machine, is git-ignored and must never be committed.
